...
The frontend is a static Vite build. Run `npm run release:package` to build and produce a deterministic integrity manifest for the generated `dist/` directory, then deploy that directory to any HTTPS static-hosting provider with SPA history fallback to `index.html`. The checked-in package contract deliberately has no hosting-provider configuration, infrastructure target, or secret; see [`docs/event-release-packaging.md`](docs/event-release-packaging.md).
