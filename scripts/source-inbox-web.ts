...
/** Loopback-only local UI; it has no dispatcher, credentials, network fetches, or persistence. */
export async function startSourceInboxWebServer(options: { readonly port?: number; readonly provider?: MediaAnalysisProvider } = {}): Promise<SourceInboxWebServer> {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === 'POST' && request.url === '/api/analyze') try { return sendJson(response, 200, { preview: await analyzeSourceInbox({ ...inputFromBody(await readJson(request)), collectedAt: new Date().toISOString() }, provider) }) } catch (error) { return sendJson(response, 400, { error: error instanceof Error ? error.message : 'Unable to analyze local submission' }) }
