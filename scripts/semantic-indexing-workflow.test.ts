...
describe('Workflow C',()=>{
 ...
 it('has one endpoint request with environment credentials and a bounded timeout',()=>{const nodes=workflow.nodes.filter((n:{type:string})=>n.type==='n8n-nodes-base.httpRequest');expect(nodes).toHaveLength(1);expect(JSON.stringify(nodes[0])).toContain('$env.SUPABASE_URL');expect(JSON.stringify(nodes[0])).toContain('$env.INDEX_SOURCE_TOKEN');expect(nodes[0].parameters.options).toEqual({timeout:95000})})
