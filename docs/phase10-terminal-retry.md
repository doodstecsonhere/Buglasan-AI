...
This implemented handler path is distinct from the [inspection-only script](../scripts/phase10-terminal-retry.ts). The script only performs the separately scoped read-only inspection and does not authorize, claim, or execute recovery. Live execution still requires explicit operator authorization and an approved deployed configuration; repository documentation, local inspection, or the presence of the handler and migration is not that authorization.
...
5. **STOP.** Execution options are intentionally rejected. No runnable live maximum-four extraction loop is supplied because deployed reconciliation safety and claim-consuming integration are unresolved. A future approved runner must make at most four sequential single extraction requests, durably reserve each before sending, and never retry ambiguous responses. The database fence must remain permanent even if local receipts are lost.
