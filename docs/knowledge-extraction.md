...
The checked-in [`supabase/config.toml`](../supabase/config.toml) sets `verify_jwt=false` for `extract-source`, because n8n does not possess a Supabase JWT and the function enforces its own token before privileged work. Public `chat` also disables JWT verification because publishable API keys are not JWTs. Both hosted functions use the `default` entry from the automatically provided `SUPABASE_SECRET_KEYS` JSON dictionary, never the local `SUPABASE_SECRET_KEY`. Local extraction validation uses `SUPABASE_SECRET_KEY` for database requests. Configure application secrets out of band, then deploy in the approved deployment phase with `supabase functions deploy extract-source --no-verify-jwt`. Do not place token values in the repository.
