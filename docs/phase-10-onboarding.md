...
3. Use `npm run phase10:manifest -- --validate path/to/file.json` or a `.jsonl` file. Exactly one positional manifest path is accepted; shell punctuation is ignored and multiple paths are rejected.
...
Each record is the existing source-ingestion payload plus `provenance` (`operator`, `reviewed_at`, `capture_note`). Fields are closed-world: unknown fields fail. `post_url` must be HTTPS and exactly `www.facebook.com/Buglasan/{posts|videos|photos}/...`; redirects, arbitrary URLs, credentials, and inferred URLs are not accepted. `post_id` is stable and unique within a manifest. Timestamps require timezone-bearing RFC 3339 values. `festival_year` is nullable and never inferred. `media_urls` is separate from text.
