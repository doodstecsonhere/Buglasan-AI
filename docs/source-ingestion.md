...
The RPC is `SECURITY DEFINER` with fixed search path and UTC timezone. Execution is revoked from public, `anon`, and `authenticated`, and granted only to `service_role`. Existing RLS and public source-read intent remain. Never expose `SUPABASE_SERVICE_ROLE_KEY` to a browser, caller, export, log, or repository.
