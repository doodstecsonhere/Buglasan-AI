...
// ---------------------------------------------------------------------------
// Unresolved citation token cleanup
//
// A recognized internal citation marker (`[Source N]`, `【Source N】`, compounds,
// or a `_(src:id)_` reference) whose target is absent from the answer's supplied
// Evidence must never be shown raw. Only the token itself is removed — adjacent
// prose and punctuation are untouched, no URL is fabricated, and no token is
// remapped onto another source. Text that does not match the Source citation
// grammar ("[Note 1]", "【Note 1】", bare "[2]") is never touched.
// ---------------------------------------------------------------------------
const HORIZONTAL_CITE_WS = '[ \\t\\u00A0\\u2000-\\u200B\\u202F\\u205F\\u3000]'
const SRC_ID_TOKEN_SOURCE = '_\\(src:\\s*[a-zA-Z0-9_-]+\\)_'
const CITATION_WITH_BOUNDARIES_RE = new RegExp(`(${HORIZONTAL_CITE_WS}*)(${SRC_ID_TOKEN_SOURCE}|${CITATION_TOKEN_SOURCE})(${HORIZONTAL_CITE_WS}*)`, 'gi')

/** Whether a recognized citation token resolves against at least one Evidence source. */
function citationTokenResolves(token: string, evidence: readonly SourceCitation[]): boolean {
  const idMatch = token.match(/^_\(src:\s*([a-zA-Z0-9_-]+)\)_$/)
  if (idMatch) return evidence.some((source) => source.id === idMatch[1])
  return (token.match(/\d+/g) ?? []).some((value) => !!evidence[Number(value) - 1])
}

/**
 * Remove every citation token that cannot resolve to the supplied Evidence, keeping
 * a single surrounding space only when the token sat between two spaced words so
 * neighbouring words can never be glued together. Deterministic and idempotent:
 * a second pass sees only resolvable tokens and changes nothing.
 */
export function stripUnresolvedCitationTokens(text: string, evidence: readonly SourceCitation[]): string {
  if (!text) return text
  return text.replace(CITATION_WITH_BOUNDARIES_RE, (match, before: string, token: string, after: string) => {
    if (citationTokenResolves(token, evidence)) return match
...
export function renderCitedContent(content: unknown, sources: unknown) {
  ...
  // A citation whose source is absent from Evidence is dropped before any parsing, so
  // the raw internal token can never surface in visible prose (the compound path below
  // keeps the resolvable references of a mixed token). Stripping first also keeps an
  // unresolvable trailing token from being mistaken for a collapsed bibliography entry.
  const strippedContent = stripUnresolvedCitationTokens(rawContent, safeSources)
  // Deterministically repair known structural defects before any parsing.
  const normalizedContent = normalizeAnswerMarkdown(strippedContent)
...
// A standalone Markdown thematic-break line (`---`, `***`, `___`) that models emit
// as a section separator. Whole-line only, so hyphens inside prose, em dashes, list
// markers, and dates are never matched.
const HORIZONTAL_RULE_LINE = /^[-*_]{3,}$/
...
export function renderMarkdownText(text: string, evidence: readonly SourceCitation[] = []): ReactNode[] {
  ...
  while (index < lines.length) {
    ...
    if (HORIZONTAL_RULE_LINE.test(line)) {
      // Structural separator sludge between generated answer sections: the rule itself
      // is never shown, but the block state is kept so the sections around it retain
      // their sensible vertical spacing.
      index += 1
      continue
    }
    ...
    while (index < lines.length && !/^#{1,6}\s+/.test(lines[index].trim()) && !/^[-*]\s+/.test(lines[index].trim()) && !/^\d+\.\s+/.test(lines[index].trim()) && !matchStandaloneSectionHeading(lines[index].trim()) && !HORIZONTAL_RULE_LINE.test(lines[index].trim()) && lines[index].trim() !== '') {
