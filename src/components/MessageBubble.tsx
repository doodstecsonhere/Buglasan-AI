...
export function renderCitedContent(content: unknown, sources: unknown) {
  const safeContent = typeof content === 'string' && content.trim() ? stripTrailingSourcesSection(content) : 'Unable to display this message.'
...
function stripTrailingSourcesSection(text: string): string {
  ...
  const trailingPattern = /(?:\n\s*)?(?:\*\*Sources\*\*|Sources:|Source:)\s*(?:\n\s*(?:\[\s*Source\s+\d+\s*\]|_\(src:\s*[a-zA-Z0-9_-]+\)_|[-*]\s*\[\s*Source\s+\d+\s*\]))*\s*$/i
  return normalized.replace(trailingPattern, '').trim()
}
