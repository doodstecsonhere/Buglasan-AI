  ...
  return <div className="app-shell flex min-h-[100dvh] flex-col overflow-hidden text-slate-900">
    ...
    <div className="mx-auto flex min-h-0 w-full flex-1 max-w-[1440px]">
      ...
      <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div className={`conversation-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-6 sm:px-6${messages.length === 0 ? ' is-empty' : ''}`} aria-live="polite"><div className="mx-auto w-full max-w-4xl"><ChatInterface messages={messages} isLoading={isLoading} messagesEndRef={messagesEndRef} isOnline={isOnline} /></div></div>
        ...
        <div className="composer-dock"><div className="mx-auto w-full max-w-4xl"><MessageInput inputRef={composerRef} onSend={handleSendMessage} disabled={isLoading} placeholder={`Ask about ${productConfig.identity.festivalName}...`} /></div></div>
      </main>
    </div>
    <AIDisclaimer />
  </div>
