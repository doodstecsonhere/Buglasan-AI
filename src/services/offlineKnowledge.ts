...
export async function answerOffline(request: ChatRequest, year: FestivalYear): Promise<ChatResponse> {
  ...
  if (isAnnouncement(request.message)) {
    ...
    return offlineResponse(content, latest ? snapshot.sources.slice(0, 1) : [], [], year, language, snapshot.freshness)
  }
  ...
  if (!events.length) return offlineResponse(`${unavailable(language)}${suffix}`, snapshot.sources, [], year, language, snapshot.freshness)
  ...
  return offlineResponse(`${cachedFacts(language)}\n${lines.join('\n')}${suffix}`, snapshot.sources, events, year, language, snapshot.freshness)
}

function offlineResponse(content: string, sources: SourceCitation[], events: Event[], year: FestivalYear, language: ChatLanguage, freshness?: FreshnessMetadata): ChatResponse {
  return { message: { id: crypto.randomUUID(), role: 'assistant', content, timestamp: new Date().toISOString(), sources, festivalYear: year }, retrievedSources: [], retrievedEvents: events, yearResolved: year, language, ...(freshness ? { freshness } : {}) }
}
...
function offlineFreshness(language: ChatLanguage, date: string) { return ({ en: `Offline cache saved ${date}. It may be stale; newer information may exist online.`, ceb: `Offline cache gi-save ${date}. Posibleng dili na kini bag-o; basin adunay mas bag-ong impormasyon online.`, fil: `Na-save ang offline cache noong ${date}. Maaaring luma na ito; maaaring may mas bagong impormasyon online.` })[language] }
